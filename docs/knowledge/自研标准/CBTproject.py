            ...
            if '# ' in line and line[0:2] == '# ':
                ...
                if '## ' in line and line[0:3] == '## ':
                    ...
                    if re.match(r"^([\d]+\.\s)", line):
                        sign_three = 3
                        num_title = line
                        priority = 'P1' if len(num_title.split('| ')) < 2 else num_title.split('| ')[-1]
                        set_md[one_title][two_title][num_title] = ['',priority,  '']
                    elif sign_three == 3:
                        """2. 对每行进行处理，如果存在链接，则需要更改格式为=HYPERLINK(url,其他内容)
                        可以判断是否存在A[B](C)D格式，若存在则转化为=HYPERLINK(C,B) \n A \n D 
                        """
                        if ('* ' in line and line[0:2] == '* ') or ('- ' in line and line[0:2] == '- '):
                            ...
                            set_md[one_title][two_title][num_title][2] = set_md[one_title][two_title][num_title][2] + line
